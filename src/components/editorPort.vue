<template>
  <!-- 编辑接口管理 -->
  <div class="wrap">
    <span class="wrapTit">编辑项目</span>
    <div class="popUpItem">
      <div>
        <span>接口模块</span>
        <el-select v-model="modulesVal">
    ...
    <div class="popUpItem">
      ...
      <span class="send pointer" @click="apiOperateTwo">保存</span>
    </div>
    <div class="main">
      <div class="mainLeft">
...
export default {
  ...
  methods: {
    getId() {
      const that = this;
      that.api_id = this.$route.query.api_id
      that.modulesVal = this.$route.query.module_id
      that.project_id = this.$route.query.project_id
      let arr = [];
      arr.push(that.api_id);
      that.$http
        .post("/api/apiOperate", {
          operate_type: "3",
          api_id: arr,
        ...
        .then(function(response) {
          ...
          that.operateName = response.data.data.name
          that.wayValue = response.data.data.req_method
          that.req_temp_host = response.data.data.req_temp_host
          that.req_relate_url = response.data.data.req_relate_url
        })
